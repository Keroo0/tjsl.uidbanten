...
const stats = (props: StatsSectionProps) => [
  ...
  },
  {
    ...
    value: props.totalBeneficiaries,
    desc: 'Jiwa terbantu',
    color: 'text-accent',
    bg: 'bg-accent/8',
  },
  {
    ...
    display: props.activeYears.length > 0 ? `${props.activeYears[0]}–${props.activeYears[props.activeYears.length - 1]}` : '2024–2026',
    ...
    <section className="py-16 bg-white border-y border-border/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {items.map(({ icon: Icon, label, value, display, desc, color, bg }, i) => (
